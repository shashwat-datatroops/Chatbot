...
def sanitize_response(response):
    ...
    # Remove LLM explanations and reasoning
    response = re.sub(r"(?i)(in your rules|according to the rules|as per your instructions|as per your rules|as requested|following your rules|per your instructions|I understand that|In the user message|from the user's message|you provided|it seems like|there might be|or confusion|assume that|considering the options|referring to|which stands for|so, the|is repeated|I will assume|that the user|is referring to)", "", response)
    
    # Clean up department-related text
    response = re.sub(r"(?i)(department:|the medical department from the user's message is:|the user's message contains a valid medical department, which is|the medical department from the user's message \"ent\" is ent\.|the medical from the user's message ent is ent\.|the medical department from the user's message is:|the medical :|the medical from the user's message is:|\"ent\" is \"ent\"|\"ent\" is ent|ent is \"ent\"|ent is ent|\"ent\"|\s*\(.*?\))", "", response)
    
    # Remove explanations about responses
    response = re.sub(r"(?i)(my response only includes|in your rules you want|therefore,|I will only include|excluding prefixes|without any prefixes|as instructed)", "", response)
    
    # Clean up other prefixes and formatting
    ...
    # Capitalize first person pronouns
...
#                 - If no name mentioned or user says "no preference", return "no preference"
...
#                 if doctor_response.lower() in ["no preference", "unknown", "none", "any", "anyone"]:
#                     department = pending_appointment.get('department', '').lower()
#                     department = department.replace("department", "").strip()

#                     if department in self.doctors and self.doctors[department]:
#                         # Select a random doctor from the department's list
#                         selected_doctor = random.choice(self.doctors[department])
#                         pending_appointment["doctor"] = f"Dr. {selected_doctor}"
#                         response = f"I'll assign you to Dr. {selected_doctor} in our {department.title()} department. When would you like to schedule this appointment?"
#                     else:
#                         # If no doctors available in that department, use a generic specialist
