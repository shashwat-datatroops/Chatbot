...
class PatientChatbot:
    ...
    def continue_appointment_booking(self, message, pending_appointment, state):
        ...
        if state == "waiting_for_department_confirmation":
            ...
            if confirmation in ["yes", "y", "yeah", "ok"]:
                ...
                if pending_appointment.get('doctor'):
                    response = f"Great! I'll book with Dr. {pending_appointment['doctor']} in the {pending_appointment['department']} department. When would you like to schedule?"
                    new_state = "waiting_for_date"
                else:
                    response = f"Which doctor would you like in the {pending_appointment['department']} department? Say 'no preference' if you don't have one."
        ...
        if state == "waiting_for_department":
            department_prompt = ChatPromptTemplate.from_template(
                """Extract ONLY the medical department from the user's message.
                User message: {message}
                ...
                IMPORTANT: Do not include any explanations, reasoning, or prefixes in your response. 
                Return ONLY the exact department name or 'unknown'. Nothing else.
                """
            )
            ...
            try:
                ...
                # More moderate cleaning to remove explanatory text but keep the department name intact
                department = re.sub(r'.*?(cardiology|dermatology|neurology|orthopedics|pediatrics|psychiatry|radiology|surgery|oncology|gynecology|urology|ent|ophthalmology).*', r'\1', department, flags=re.IGNORECASE | re.DOTALL)
                department = department.strip()
                
                if department == "unknown" or not department:
                ...
                # Set department in pending_appointment
            ...
            # Check for no preference directly without going through LLM for simple cases
            if message.strip().lower() in ["no preference", "none", "no", "any doctor", "doesn't matter", "don't care", "any"]:
                department = pending_appointment.get('department', '')
                
                # Apply same improved regex approach for department cleaning
                department = re.sub(r'.*?(cardiology|dermatology|neurology|orthopedics|pediatrics|psychiatry|radiology|surgery|oncology|gynecology|urology|ent|ophthalmology).*', r'\1', department, flags=re.IGNORECASE | re.DOTALL)
                department = department.strip()
                
                # Update the department in pending_appointment
                pending_appointment['department'] = department

                if department in self.doctors and len(self.doctors[department]) > 0:
                    ...
                    pending_appointment["doctor"] = doctor_name
                    response = f"I'll assign you to {doctor_name} in our {department.title()} department. When would you like to schedule this appointment?"
            ...
            doctor_prompt = ChatPromptTemplate.from_template(
                ...
                - Return ONLY the exact name from the message, without modifications

                IMPORTANT: Your entire response should be ONLY the name or "no preference". 
            ...
            try:
                ...
                doctor_response = re.sub(r"(?i)(in your rules|according to the rules|as per your instructions|as per your rules|as requested|following your rules|per your instructions|I understand that|In the user message|from the user's message|you provided|it seems like|there might be|or confusion|assume that|considering the options|referring to|which stands for|so, the|is repeated|I will assume|that the user|is referring to)", "", doctor_response)
                doctor_response = re.sub(r"(?i)(my response only includes|in your rules you want|therefore,|I will only include|excluding prefixes|without any prefixes|as instructed)", "", doctor_response)
                doctor_response = re.sub(r"```", "", doctor_response).strip()
                ...
                if doctor_response.lower() in ["no preference", "unknown", "none"]:
                    ...
                    doctor_name = re.sub(r"(?i)dr\.?\s*", "", doctor_name).strip()
                    doctor_name = re.sub(r"(?i)doctor\s*", "", doctor_name).strip()
                    pending_appointment["doctor"] = doctor_name
                    ...
                    response = f"Great! I'll book an appointment with Dr. {doctor_name} in the {department.title()} department. When would you like to schedule this appointment?"
            ...
            date_prompt = ChatPromptTemplate.from_template(
                ...
                - Return ONLY the date in the exact specified format
                - Do not include any reasoning or explanation

                Examples:
                User: "Next Monday" -> "May 5, 2025"
                User: "May 5th" -> "May 5, 2025"
                User: "June 1" -> "June 1, 2025"
                
                IMPORTANT: Your response should contain ONLY the date or "unknown".
                Nothing else. No explanations.
                """
            )
            ...
            try:
                ...
                # Clean unwanted text from response
                date_response = re.sub(r"(?i)(in your rules|according to the rules|as per your instructions|as per your rules|as requested|following your rules|per your instructions|I understand that|based on the message|following the rules|the date in the format)", "", date_response)
                date_response = re.sub(r"(?i)(mixtral:|llm:|bot:|assistant:)", "", date_response)
                ...
                # Get doctor info and format it properly
                doctor_info = pending_appointment.get('doctor', '')
                if doctor_info:
                    doctor_info = re.sub(r"(?i)^dr\.?\s*", "", doctor_info).strip()
                    doctor_info = re.sub(r"(?i)mixtral:\s*", "", doctor_info).strip()
                    doctor_info = re.sub(r"(?i)```", "", doctor_info).strip()
                    doctor_info = re.sub(r"(?i)(in your rules|according to the rules|as per your instructions|as per your rules|as requested|following your rules|per your instructions|I understand that|In the user message|from the user's message|you provided|it seems like|there might be|or confusion|assume that|considering the options|referring to|which stands for|so, the|is repeated|I will assume|that the user|is referring to)", "", doctor_info)
                    doctor_info = re.sub(r"(?i)(my response only includes|in your rules you want|therefore,|I will only include|excluding prefixes|without any prefixes|as instructed)", "", doctor_info)
                    doctor_info = f"Dr. {doctor_info}"
                    pending_appointment['doctor'] = doctor_info

                # Get department and format it properly
                department = pending_appointment.get('department', '')
                ...
                response = f"Perfect! I'll schedule your {department.title()} appointment with {doctor_info} for {date}. What time would you prefer? Our available slots are between 9:00 AM to 4:30 PM."
                ...
                response = "I couldn't understand the date. Could you please provide a specific date for your appointment? For example, 'May 5, 2025'."
                return {"response": response}
            
        elif state == "waiting_for_time":
            time_prompt = ChatPromptTemplate.from_template(
                    """Extract ONLY the time from this message and format it as a standard 12-hour time:
                    Message: {message}

                    Return ONLY the time in format "HH:MM AM/PM" (e.g., "10:00 AM").
                    If no clear time is provided, return "unknown".
                    
                    IMPORTANT: Your entire response should be ONLY the time or "unknown".
            ...
            try:
                ...
                # Clean unwanted text from response
                requested_time = re.sub(r"(?i)(in your rules|according to the rules|as per your instructions|as per your rules|as requested|following your rules|per your instructions|I understand that|based on the message|following the rules|the time extracted from)", "", requested_time)
                requested_time = re.sub(r"(?i)(mixtral:|llm:|bot:|assistant:)", "", requested_time)
                requested_time = re.sub(r'"', '', requested_time)
                
                time_match = re.search(r'(\d{1,2}:\d{2} [AP]M)', requested_time, re.IGNORECASE)
                if not time_match:
                    # Try to find other time formats
                    alt_time_match = re.search(r'(\d{1,2})(?::(\d{2}))?\s*([ap]\.?m\.?)', requested_time, re.IGNORECASE)
                    if alt_time_match:
                        hour = alt_time_match.group(1)
                        minute = alt_time_match.group(2) or "00"
                        am_pm = alt_time_match.group(3).upper().replace(".", "")
    ...
    def _format_appointment_confirmation(self, details):
        ...
        doctor = details.get('doctor', 'Not specified')
        department = details.get('department', 'Not specified')
        date = details.get('date', 'Not specified')
        time = details.get('time', 'Not specified')
        phone = details.get('phone_number', 'Not provided')
        ...
        if doctor != "Not specified":
            doctor = re.sub(r"(?i)^dr\.?\s*", "", doctor).strip()
            doctor = re.sub(r"(?i)mixtral:\s*", "", doctor).strip()
            doctor = re.sub(r"(?i)```", "", doctor).strip()
            doctor = f"Dr. {doctor}"
